...
public class BoardView{
    ...
    public BoardView(int inputSize,String type){
        ...
        System.out.println(ANSI_PURPLE+STAR.repeat(2)+"  "+type+"  "+STAR.repeat(2)+ANSI_RESET+"\r");
        this.gridStructure = new GridStructure(this.boardSize);

    }

    public int getX(String PlayerColor){
        System.out.println("\r\n"+ANSI_PURPLE+"Insert new "+PlayerColor+" piece (x coordinate): "+ANSI_RESET+"\r");
        try {
           return new Scanner(System.in).nextInt();
        }catch (InputMismatchException e){
            System.out.println(ANSI_RED+"Invalid coordinate "+ANSI_RESET+new String(Character.toChars(0x1F6AB))+"\r");
           return this.getX(PlayerColor);
        }
    }

    public int getY(String PlayerColor){
        System.out.println("\r\n"+ANSI_PURPLE+"Insert new "+PlayerColor+" piece (y coordinate): "+ANSI_RESET+"\r");
        try {
            return new Scanner(System.in).nextInt();
        }catch (InputMismatchException e){
            System.out.println(ANSI_RED+"Invalid coordinate "+ANSI_RESET+new String(Character.toChars(0x1F6AB))+"\r");
